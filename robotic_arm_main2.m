...
fprintf("Welcome to the Robotic Arm Simulator.\n") ;
nn = -1;

while nn < 0 || nn == 1
    nn = input('How many links would you like to use? (Enter zero for default) \nNumber of Links must be greater than 2: ') ;
    if nn < 0
...
while ~(yn == 'y' || yn == 'Y' || yn == 'n' || yn == 'N')
    yn = input('Would you like to save a gif of the simulation (y/n): ', 's') ;
...
if yn == 'y' || yn == 'Y'
    gif_name = input('Enter a name for the file: ', 's') ;
end
...
if yn == 'Y' || yn == 'y'
    ...
    gif(gif_name, 'DelayTime', 1/30, 'frame', gcf)
end
